...
use transact_parser::{parse_format, Transaction, ParserError, Result};
...
fn read_transactions(filename: &str, format: &str) -> Result<Vec<Transaction>> {
    let file = File::open(filename)
        .map_err(|e| ParserError::Io(io::Error::new(
            io::ErrorKind::Other, 
...
fn compare_transactions(
    ...
    if transactions1.len() != transactions2.len() {
        println!("❌ Разное количество транзакций: {} vs {}", 
                 transactions1.len(), transactions2.len());
        return false;
    }
    ...
    for (i, (t1, t2)) in transactions1.iter().zip(transactions2.iter()).enumerate() {
        ...
        if t1.from_user_id != t2.from_user_id {
            mismatches.push(format!("from_user_id ({} vs {})", t1.from_user_id, t2.from_user_id));
        }
        if t1.to_user_id != t2.to_user_id {
            mismatches.push(format!("to_user_id ({} vs {})", t1.to_user_id, t2.to_user_id));
        }
...
fn main() -> Result<()> {
    ...
    println!("📁 Чтение файла 1: '{}' (формат: {})", cli.file1, cli.format1);
    let transactions1 = read_transactions(&cli.file1, &cli.format1)?;
    
    println!("📁 Чтение файла 2: '{}' (формат: {})", cli.file2, cli.format2);
